...
function requiresc(path, parent, orig) {
  var resolved = requiresc.resolve(path);

  // lookup failed
  if (null == resolved) {
    orig = orig || path;
    parent = parent || 'root';
    var err = new Error('Failed to requiresc "' + orig + '" from "' + parent + '"');
    err.path = orig;
    err.parent = parent;
    err.requiresc = true;
    throw err;
  ...
  var module = requiresc.modules[resolved];

  // perform real requiresc()
  ...
  if (!module._resolving && !module.exports) {
    ...
    module.call(this, mod.exports, requiresc.relative(resolved), mod);
...
requiresc.resolve = function(path) {
  ...
  for (var i = 0; i < paths.length; i++) {
    var path = paths[i];
    if (requiresc.modules.hasOwnProperty(path)) return path;
    if (requiresc.aliases.hasOwnProperty(path)) return requiresc.aliases[path];
  }
};
...
requiresc.normalize = function(curr, path) {
  var segs = [];
...
requiresc.register = function(path, definition) {
  requiresc.modules[path] = definition;
};
...
requiresc.alias = function(from, to) {
  if (!requiresc.modules.hasOwnProperty(from)) {
    throw new Error('Failed to alias "' + from + '", it does not exist');
  }
  requiresc.aliases[to] = from;
};

/**
 * Return a requiresc function relative to the `parent` path.
 *
 * @param {String} parent
 * @return {Function}
 * @api private
...
requiresc.relative = function(parent) {
  var p = requiresc.normalize(parent, '..');
  ...
  /**
   * The relative requiresc() itself.
   */

  function localrequiresc(path) {
    var resolved = localrequiresc.resolve(path);
    return requiresc(resolved, parent, path);
  }
  ...
  localrequiresc.resolve = function(path) {
    var c = path.charAt(0);
    if ('/' == c) return path.slice(1);
    if ('.' == c) return requiresc.normalize(p, path);
  ...
  localrequiresc.exists = function(path) {
    return requiresc.modules.hasOwnProperty(localrequiresc.resolve(path));
  };

  return localrequiresc;
};
requiresc.register("component-emitter/index.js", function(exports, requiresc, module){
...
requiresc.register("side-comments/js/main.js", function(exports, requiresc, module){
var _ = requiresc('./vendor/lodash-custom.js');
var Section = requiresc('./section.js');
var Emitter = requiresc('emitter');
var $ = jQuery;
...
SideComments.prototype.showComments = function() {
  //console.log(this.activeSection.$el.parent());
  //this.$el.addClass('side-comments-open');
  if(this.activeSection){
    this.activeSection.$el.parent().addClass('side-comments-open');
  }
};
...
SideComments.prototype.hideComments = function() {
  ...
  if (this.activeSection) {
    this.activeSection.$el.parent().removeClass('side-comments-open');
...
requiresc.register("side-comments/js/section.js", function(exports, requiresc, module){
var _ = requiresc('./vendor/lodash-custom.js');
var Template = requiresc('../templates/section.html');
var CommentTemplate = requiresc('../templates/comment.html');
var mobileCheck = requiresc('./helpers/mobile-check.js');
var $ = jQuery;
...
Section.prototype.markerClick = function( event ) {
	...
	this.select();
  this.$el.find('.side-comment').removeClass('new-comments');
};
...
Section.prototype.insertComment = function( comment ) {
  ...
  this.$el.find('.side-comment').addClass('new-comments');
...
requiresc.register("side-comments/js/vendor/lodash-custom.js", function(exports, requiresc, module){
/**
 * @license
...
;(function() {
   ...
   * don't requiresc escaping in HTML and have no special meaning unless they're part
  ...
  function template(text, data, options) {
    ...
    text.replace(reDelimiters, function(match, escapeValue, interpolateValue, esTemplateValue, evaluateValue, offset) {
      ...
      // the JS engine embedded in Adobe products requirescs returning the `match`
...
requiresc.register("side-comments/js/helpers/mobile-check.js", function(exports, requiresc, module){
...
requiresc.register("side-comments/templates/section.html", function(exports, requiresc, module){
...
requiresc.register("side-comments/templates/comment.html", function(exports, requiresc, module){
...
requiresc.alias("component-emitter/index.js", "side-comments/deps/emitter/index.js");
requiresc.alias("component-emitter/index.js", "emitter/index.js");

requiresc.alias("side-comments/js/main.js", "side-comments/index.js");
